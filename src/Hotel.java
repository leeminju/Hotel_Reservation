...
public class Hotel {
    ...
    private void CheckMyReservation() {
        List<UUID> temp = current_logined_customer.reservationList; // 로그인된 예약리스트를 temp에 담아줌
        for (int i = 0; i < temp.size(); i++) {
            UUID uuid = temp.get(i);
            Reservation reservation = reservationMap.get(uuid);

            System.out.println("예약 번호: " + reservation.reserved_ID);
            System.out.println("예약자 명: " + reservation.customer_name);
            System.out.println("예약자 전화번호: " + reservation.customer_phone);
            System.out.println("예약한 객실: " + reservation.room.size);
